...
def convert_to_ts(df_orig, country=None):
    ...
    # new
    df_time['month'] = df_time['year_month'].str.split('-').apply(lambda x: x[1])
    df_time = df_time[df_time['date'] >= '12-01-2017']
        
    df_time = pd.concat([df_time[df_time.columns.tolist()[:-1]], pd.get_dummies(df_time.month, prefix='month')], axis=1)
    # add months as 0 variables
    for i in range(1, 13):
        month = f'month_{i:02d}'
        if month not in df_time.columns:
            df_time[month] = 0
            df_time[month] = df_time[month].astype(int)
    
    df_time = df_time[['date', 'purchases', 'unique_invoices', 'unique_streams', 'total_views',
